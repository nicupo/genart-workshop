...
// ============================================================
// YOUR ARTIST INFO — edit these two lines, nothing else:
// const ARTIST_NAME = "yourname";
// const ARTIST_TITLE = "Untitled";
//
// You do NOT need to rename any function anywhere in this file.
// Everything below runs in its own private space, so your
// artwork never collides with anyone else's even if the function
// inside keeps the exact same name for every artist.
//
...
(function () {
  ...
  const ARTIST_NAME = "yourname";
  const ARTIST_TITLE = "Untitled"; //title of your work

  function drawArtwork(g, t, w, h) {
    // Inputs:
  ...
  // Registers your work with the collective piece. It won't do
  // anything while you're working here (no ARTISTS registry
  // exists in this standalone folder, so it just skips itself).
  if (typeof ARTISTS !== "undefined") {
    ARTISTS.push({ name: ARTIST_NAME, title: ARTIST_TITLE, draw: drawArtwork });
  }

  // used only by the standalone preview below — always leave
  // these two lines exactly as they are
  PREVIEW_DRAW = drawArtwork;
  PREVIEW_LABEL = `${ARTIST_NAME} - ${ARTIST_TITLE}`;

})();


// ============================================================
// STANDALONE PREVIEW — nothing to touch below this line. It
// runs your artwork at full size, styled the same way the final
// collective piece will look, so what you see here while you
// build is what it'll look like on the day. Nothing here gets
// copied over — only your artwork above does.
// ============================================================
...
function draw() {
  previewT += 0.003;
  PREVIEW_DRAW(previewBuffer, previewT, width, height);
